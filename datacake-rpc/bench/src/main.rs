...
async fn run_client() -> Result<()> {
    let mut client = EchoClient::connect("http://127.0.0.1:9001").await?;
    ...
    println!("Tonic took: {:?}, {:?}/req", start.elapsed(), start.elapsed() / 1_000);

    let connect = "127.0.0.1:9000".parse::<SocketAddr>().unwrap();
    ...
    println!("Datacake took: {:?}, {:?}/req", start.elapsed(), start.elapsed() / 1_000);
