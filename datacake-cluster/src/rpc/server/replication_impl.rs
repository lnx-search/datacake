...
use crate::rpc::datacake_api::{Empty, FetchDocs, FetchedDocs, GetState, KeyspaceOrSwotSet, KeyspaceInfo};
...
impl<S: Storage + Send + Sync + 'static> ReplicationApi for ReplicationService<S> {
    async fn poll_keyspace(&self, _request: Request<Empty>) -> Result<Response<KeyspaceInfo>, Status> {
        let keyspace_timestamps = self.group
            .serialize_keyspace_counters()
            .map_err(|e| Status::internal(e.to_string()))?;

        Ok(Response::new(KeyspaceInfo { keyspace_timestamps }))
    }
    ...
    async fn get_state(&self, request: Request<GetState>) -> Result<Response<KeyspaceOrSwotSet>, Status> {
        ...
        let set_data = keyspace.serialize()
            .await
            .map_err(|e| Status::internal(e.to_string()))?;

        Ok(Response::new(KeyspaceOrSwotSet { last_updated, set_data }))
    }
